...
module.exports = function(config) {
    ...
    config.set({
        ...
        files: [
            ...
            {pattern: 'components/brandkit/fonts/*.woff', included: false, served: true },
            {pattern: 'css/*.css', included: false, served: true },
            {pattern: 'js/**/*.js', included: false}, // make everything in our project available (but don't load it)
            'test/require.config.js' // require config needs to be loaded into the browser
        ].concat(config.getDefaultFilesForDX('*', 'test/unit/spec')), // concat together with all the default paths
